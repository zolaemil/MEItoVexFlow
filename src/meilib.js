...
MeiLib.MeiDoc.prototype.parseALTs = function() {
  ...
  for (var i = 0; i < apps.length; i++) {
    ...
    for (var j = 0; j < altitems.length; j++) {
      ...
      AppsItem.altitems[varXMLID] = new MeiLib.Variant(varXMLID, tagname,
          source, resp, n);
    }
...
MeiLib.MeiDoc.prototype.initSectionView = function(altReplacements) {
  ...
  $(alts).each(function(i, alt) {
    ...
    if (replacement) {
      ...
      if (!alt_item)
        throw new MeiLib.RuntimeError(
            'MeiLib.MeiDoc.prototype.initSectionView():E01',
            "Cannot find <lem>, <rdg>, <sic>, or <corr> with @xml:id '"
                + alt_item_xml_id + "'.");
    ...
    var PIEnd = xmlDoc.createProcessingInstruction('MEI2VF', 'rdgEnd="'
            + alt_xml_id + '"');
  ...
  for (altID in sectionplaneUpdate) {
    ...
    if (altgroup) {
      ...
      for (i = 0; i < altgroup.length; i++) {
        altID__ = altgroup[i];
        alt_instance2insert__ = corresponding_alt_item(
            this.ALTs[altID__].altitems, alt_instance2insert);
