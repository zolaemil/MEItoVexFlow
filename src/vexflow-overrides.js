...
Vex.Flow.Curve.prototype.renderCurve = function(params) {
  ...
  ctx.bezierCurveTo(first_x + cp_spacing + cps[0].x, first_y
          + (cps[0].y * params.direction), last_x - cp_spacing
          + cps[1].x, last_y + (cps[1].y * params.direction), last_x,
      last_y);
  ctx.bezierCurveTo(last_x - cp_spacing + cps[1].x, last_y
          + ((cps[1].y + thickness) * params.direction), first_x
          + cp_spacing + cps[0].x, first_y
          + ((cps[0].y + thickness) * params.direction), first_x,
      first_y);
  ctx.stroke();
  ctx.closePath();
  ctx.fill();
...
Vex.Flow.Barline = (function() {
  ...
  Vex.Inherit(Barline, Vex.Flow.StaveModifier, {
        ...
        drawVerticalBar : function(stave, x, double_bar) {
          if (!stave.context)
            throw new Vex.RERR("NoCanvasContext",
          ...
          // (THICKNESS / 2);
          var bottom_line = stave.getYForLine(stave.options.num_lines
              - 1)
              + (THICKNESS / 2) - 1;

          if (double_bar)
            stave.context.fillRect(x - 3, top_line, 1, bottom_line
                    - top_line + 1);
          stave.context.fillRect(x, top_line, 1, bottom_line
                  - top_line + 1);
        },
        ...
        drawVerticalEndBar : function(stave, x) {
          ...
          stave.context.fillRect(x - 5, top_line, 1, bottom_line
                  - top_line + 1);
          stave.context.fillRect(x - 2, top_line, 3, bottom_line
                  - top_line + 1);
        },
        ...
        drawRepeatBar : function(stave, x, begin) {
          ...
          stave.context.fillRect(x + x_shift, top_line, 1,
          ...
          stave.context.arc(dot_x, dot_y, dot_radius, 0, Math.PI * 2,
              false);
          stave.context.fill();
...
Vex.Flow.Annotation = (function() {
  ...
  Vex.Inherit(Annotation, Modifier, {
    ...
    draw : function() {
      if (!this.context)
        throw new Vex.RERR("NoContext",
            "Can't draw text annotation without a context.");
      if (!this.note)
        throw new Vex.RERR("NoNoteForAnnotation",
      ...
      this.context.save();
      this.context.setFont(this.font.family, this.font.size,
          this.font.weight);
      ...
      if (this.vert_justification == Annotation.VerticalJustify.BOTTOM) {
        ...
        if (has_stem) {
          var stem_base = (this.note.stem_direction === 1
              ? stem_ext.baseY
              : stem_ext.topY);
          y = Vex
              .Max(y, stem_base
                      + (spacing * (this.text_line + 2)));
        }
        ...
        y = Vex.Min(this.note.stave.getYForTopText(this.text_line),
            this.note.ys[0] - 10);
        if (has_stem) {
          y = Vex.Min(y, (stem_ext.topY - 5)
                  - (spacing * this.text_line));
        }
      } else/* CENTER_STEM */
      {
  ...
  StaveTie.prototype = {
    ...
    setNotes : function(notes) {
      ...
      if (notes.first_indices.length != notes.last_indices.length)
        throw new Vex.RuntimeError("BadArguments",
            "Tied notes must have similar" + " index sizes");
    ...
    renderTie : function(params) {
      ...
      for (var i = 0; i < this.first_indices.length; ++i) {
        var cp_x = ((params.last_x_px + last_x_shift) + (params.first_x_px + first_x_shift))
            / 2;
        var first_y_px = params.first_ys[this.first_indices[i]]
            + y_shift;
        var last_y_px = params.last_ys[this.last_indices[i]] + y_shift;

        if (isNaN(first_y_px) || isNaN(last_y_px))
          throw new Vex.RERR("BadArguments",
              "Bad indices for tie rendering.");

        var top_cp_y = ((first_y_px + last_y_px) / 2)
            + (cp1 * params.direction);
        var bottom_cp_y = ((first_y_px + last_y_px) / 2)
            + (cp2 * params.direction);

        ctx.beginPath();
        ctx.moveTo(params.first_x_px + first_x_shift, first_y_px);
        ctx.quadraticCurveTo(cp_x, top_cp_y, params.last_x_px
                + last_x_shift, last_y_px);
        ctx.quadraticCurveTo(cp_x, bottom_cp_y, params.first_x_px
                + first_x_shift, first_y_px);
    ...
    renderText : function(first_x_px, last_x_px) {
      ...
      this.context.setFont(this.font.family, this.font.size,
          this.font.style);
      this.context.fillText(this.text, center_x
              + this.render_options.text_shift_x,
          (this.first_note || this.last_note).getStave()
              .getYForTopText()
              - 1);
      this.context.restore();
    },
    ...
    draw : function() {
      ...
      if (first_note) {
        first_x_px = first_note.getTieRightX()
            + this.render_options.tie_spacing;
      ...
      if (last_note) {
        last_x_px = last_note.getTieLeftX()
            + this.render_options.tie_spacing;
