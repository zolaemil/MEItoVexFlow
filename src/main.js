var MEI2VF = (function(m2v, VF, $, undefined) {
  ...
  m2v.Viewer = function(opts) {
    ...
    if (!opts) {
      throw new m2v.RUNTIME_ERROR('MEI2VF.RERR.NoConfig',
          'No config passed to Renderer.');
    }

    if (!opts.xmlDoc) {
      throw new m2v.RUNTIME_ERROR('MEI2VF.RERR.MissingData',
          'No XML document passed to Renderer.');
    }
    ...
    if (!firstScoreDef) {
      throw new m2v.RUNTIME_ERROR('MEI2VF.RERR.BadMEIFile',
          'No <scoreDef> found in config.data.');
    }

    var cfg = $.extend(true, {}, this.defaults, m2v.util
            .getMEIPageConfig(firstScoreDef), opts);
  ...
  m2v.render_notation = function(xmlDoc, target, width, height, backend,
      options) {
  ...
  m2v.util = {
    ...
    testNS : function() {
      ...
      for (i = 0; i < 500; i += 1) {
        scoredef = xmlDoc.getElementsByTagName(
            'http://www.music-encoding.org/ns/mei', e);
    ...
    getMEIPageConfig : function(firstScoreDef) {
      ...
      return {
        ...
        page_margin_top : (isNaN(+obj['page.topmar']))
            ? undefined
            : +obj['page.topmar'],
        page_margin_left : (isNaN(+obj['page.leftmar']))
            ? undefined
            : +obj['page.leftmar'],
        page_margin_right : (isNaN(+obj['page.rightmar']))
            ? undefined
            : +obj['page.rightmar']
      };
    },
     ...
     *            the canvas context
     * @param scale
     *            {Number} the scale ratio. 1 means 100%
     */
    ...
    drawBoundingBoxes : function(ctx, options) {
      ...
      if (options.voices && options.voices.data) {
        $.each(options.voices.data, function(i, voices) {
          if (voices && voices.staveVoices
              && voices.staveVoices.all_voices) {
            $.each(voices.staveVoices.all_voices,
                function(i, voice) {
