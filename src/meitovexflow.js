...
var MEI2VF = (function(m2v, VF, $, undefined) {
  ...
  m2v.Renderer = function(config) {
    if (config) this.init(config).process().draw();
    return this;
  };
  ...
  m2v.Renderer.prototype = {
    ...
    defaults : {
      ...
      // both (space separated); some of the objects are passed directly to vexFlow (which
    ...
    init : function(config) {
      ...
      /**
       * init config: properties in the config object override MEI
       * staffDef attributes which override the defaults
       */
      me.cfg = $.extend(true, {}, me.defaults, config);

      // TODO move this to main
      me.printSpaceW = Math.floor(me.cfg.page_width / me.cfg.page_scale
          - me.cfg.page_margin_left - me.cfg.page_margin_right)
          - 1;
    ...
    processScoreDef : function(scoredef) {
      ...
      if (systemLeftmar) me.currentSystemMarginLeft = +systemLeftmar;
      ...
      switch (element.localName) {
        ...
        default :
          throw new m2v.RUNTIME_ERROR('MEI2VF.RERR.NotSupported',
              'Element <' + children[i].localName
                  + '> is not supported in <scoreDef>');
      }
    },
    ...
    processStaffGrp : function(staffGrp, isChild) {
      ...
      $(staffGrp).children().each(function(i, childElement) {
        var childRange = me.processStaffGrp_child(i, childElement);
        Vex.LogDebug('processStaffGrp() {1}.{a}: ' 
            + 'childRange.first_n:' + childRange.first_n 
            + ' childRange.last_n:' + childRange.last_n);
        if (i === 0) range.first_n = childRange.first_n;
    ...
    setConnectorModels : function(staffGrp, range, isChild) {
      ...
      Vex.LogDebug('setConnectorModels() {2}: symbol:' + symbol
          + ' range.first_n:' + first_n + ' range.last_n:' + last_n);
      ...
      // # left auto line, only attached to
    ...
    startSystem : function(measure) {
      ...
      while (i--) {
        if (me.currentStaffInfos[i]) me.currentStaffInfos[i][updateFn]();
      }
    },

    initStaffYs : function() {
      var me = this, currentSystemY, currentStaffY, lowestYCandidate, i, j, isFirstStaff = true;
      currentSystemY = (me.currentSystem === 1)
          ? me.cfg.page_margin_top
          : me.currentLowestY + me.cfg.systemSpacing;
      currentStaffY = 0;
      for (i = 1, j = me.currentStaffInfos.length; i < j; i += 1) {
        if (me.currentStaffInfos[i]) {
          currentStaffY += (isFirstStaff) ? 0
              : me.currentStaffInfos[i].spacing || me.cfg.staveHeight + me.cfg.staveSpacingAbove;
      ...
      if (lowestYCandidate > me.currentLowestY) me.currentLowestY = lowestYCandidate;
    },
    ...
    getNext : function(currentElement) {
      ...
      if (next) return next.firstChild;
    },
    ...
    addMissingMeasureWidths : function(widths) {
      ...
      nonSpecifiedMeasureWidth = Math.floor((me.printSpaceW
          - me.currentSystemMarginLeft - totalSpecifiedMeasureWidth)
          / nonSpecified_n);
      for (i = 0, j = widths.length; i < j; i += 1) {
        if (widths[i] === null) widths[i] = nonSpecifiedMeasureWidth;
      }
      return widths;
    },
    ...
    processEnding : function(element) {
      ...
      for (i = 0, j = sectionChildren.length; i < j; i += 1) {
        me.currentVoltaType = {};
        if (i === 0) me.currentVoltaType.start = $(element).attr('n');
        if (i === j - 1) me.currentVoltaType.end = '1';
        me.processSectionChild(sectionChildren[i]);
      }
    ...
    processMeasure : function(element) {
      ...
      $(element).find('staff').each(function() {
        me.processStaffInMeasure(this, measure_n, left_barline,
            right_barline, atSystemStart, atSystemTop,
            currentStaveVoices, currentMeasureWidth);
        atSystemTop = false;
      });
      ...
      if (atSystemStart) {
        me.startConnectors.createVexFromModels(
            me.allVexMeasureStaffs[measure_n], null, null,
            me.currentSystem);
      }
      me.inlineConnectors.createVexFromModels(
          me.allVexMeasureStaffs[measure_n], left_barline,
          right_barline);
    ...
    extract_tempoElements : function(element, measure) {
      ...
      $(element).find('tempo').each(function(i, tempoElement) {
            ...
            if (vo) vexTempo.setShiftY(+vo * me.halfLineDistance);
            offsetX = (staff.getModifierXShift() > 0) ? -14 : 14;
            if (staff.hasTimeSignature) offsetX -= 24;
            if (ho) offsetX += +ho * me.halfLineDistance;
    ...
    processStaffInMeasure : function(staff_element, measure_n,
        left_barline, right_barline, atSystemStart, atSystemTop,
        currentStaveVoices, currentMeasureWidth) {
      var me = this, staff, staff_n, layers, anchoredTexts, readEvents, layer_events, labelText;

      staff_n = +$(staff_element).attr('n');

      staff = me.createVexStaff(measure_n, staff_n, left_barline,
          right_barline, currentMeasureWidth, atSystemStart,
          atSystemTop);
      ...
      anchoredTexts = $(staff_element).children('anchoredText').each(
          function(i, anchoredText) {
      ...
      readEvents = function(i, element) {
        var event = me.processElement(element, this, measure_n,
            staff_n, staff);
     ...
     */
    createVexStaff : function(measure_n, staff_n, left_barline,
        right_barline, currentMeasureWidth, atSystemStart, atSystemTop) {
      var me = this, staffdef, staff, renderWith, currentStaffInfo;
      if (!staff_n) {
        throw new m2v.RUNTIME_ERROR('MEI2VF.RERR.BadArgument',
            'Cannot render staff without attribute "n".');
      }

      currentStaffInfo = me.currentStaffInfos[staff_n];

      staff = new VF.Stave(me.currentMeasureX,
          currentStaffInfo.absoluteY, currentMeasureWidth,
          me.cfg.staff);
      ...
      if (currentStaffInfo.showClefCheck()) staff.addClef(currentStaffInfo.getClef());
      if (currentStaffInfo.showKeysigCheck()) staff.addKeySignature(currentStaffInfo.getKeySpec());
      if (currentStaffInfo.showTimesigCheck()) {
        staff.addTimeSignature(currentStaffInfo.getTimeSig());
        staff.hasTimeSignature = true;
      }

      staff.setBegBarType(left_barline
          ? m2v.tables.barlines[left_barline]
          : VF.Barline.type.NONE);
      if (right_barline) staff.setEndBarType(m2v.tables.barlines[right_barline]);

      if (atSystemTop) {
        if (atSystemStart && me.cfg.autoMeasureNumbers && measure_n !== 1) staff.setMeasure(measure_n);
        if (me.currentVoltaType) me.addStaffVolta(staff);
      }
      if (atSystemStart) me.addStaffLabel(staff, staff_n);
    ...
    addStaffVolta : function(staff) {
      ...
      if (volta.start) staff.setVoltaType(Vex.Flow.Volta.type.BEGIN, volta.start + '.', 30, 0);
      if (volta.end) staff.setVoltaType(Vex.Flow.Volta.type.END, "", 30, 0);
      if (!volta.start && !volta.end) staff.setVoltaType(Vex.Flow.Volta.type.MID, "", 30, 0);
    },

    addStaffLabel : function(staff, staff_n) {
      var me = this, labelText = (me.currentSystem === 1)
    ...
    createVexVoice : function(voice_contents, staff_n) {
      ...
      if (!$.isArray(voice_contents)) {
        throw new m2v.RUNTIME_ERROR('BadArguments',
            'me.createVexVoice() voice_contents argument must be an array.');
      }
    ...
    resolveUnresolvedTimestamps : function(layer, staff_n, measure_n) {
      ...
      // (measure, staff, layer):
      if (!measure_n) 
        throw new m2v.RUNTIME_ERROR('MEI2VF.RERR.me.extract_events:',
            '<measure> must have @n specified');
      staff_n = staff_n || 1;
      refLocationIndex = measure_n + ':' + staff_n + ':'
          + ($(layer).attr('n') || '1');
      if (me.unresolvedTStamp2[refLocationIndex]) {
        $(me.unresolvedTStamp2[refLocationIndex]).each(
            function(i, eventLink) {
     ...
     * objects
     */
    extract_linkingElements : function(measure, element_name,
        eventlink_container) {
      var me = this;
      ...
      var local_tstamp2id = function(tstamp, lnkelem, measure) {
        ...
        if (!layer) {
          ...
          if (layer_candid && !layer_candid.attr('n'))
            layer = layer_candid;
          if (!layer)
            throw new m2v.RUNTIME_ERROR(
                'MEI2VF.RERR.extract_linkingElements:E01',
                'Cannot find layer');
        }
        var staffdef = me.currentStaffInfos[stffinf.staff_n];
        if (!staffdef)
          throw new m2v.RUNTIME_ERROR(
              'MEI2VF.RERR.extract_linkingElements:E02',
              'Cannot determine staff definition.');
        var meter = staffdef.meter;
        if (!meter.count || !meter.unit)
          throw new m2v.RUNTIME_ERROR(
              'MEI2VF.RERR.extract_linkingElements:E03',
              "Cannot determine meter; missing or incorrect @meter.count or @meter.unit.");
      ...
      $.each(link_elements, function(i, lnkelem) {
        ...
        if (element_name === 'hairpin' && !atts.form) {
          throw new m2v.RUNTIME_ERROR(
              'MEI2VF.RERR.BadArguments:extract_linkingElements',
              '@form is mandatory in <hairpin> - make sure the xml is valid.');
        }
        ...
        if (endid) {
          ...
          if (tstamp2) {
            ...
            if (measures_ahead > 0) {
              ...
              var refLocationIndex = target_measure_n.toString()
                  + ':' + staffinfo.staff_n + ':' + staffinfo.layer_n;
              if (!me.unresolvedTStamp2[refLocationIndex]) 
    ...
    processElement : function(element, parent_layer, measure_n, staff_n,
        staff) {
      var me = this;
      switch (element.localName) {
        case 'rest' :
          return me.processRest(element, staff, parent_layer,
              measure_n, staff_n);
        case 'mRest' :
          return me.processmRest(element, staff, parent_layer,
              measure_n, staff_n);
        case 'space' :
          return me.processSpace(element, staff, parent_layer,
              measure_n, staff_n);
        case 'note' :
          return me.processNote(element, staff, parent_layer,
              measure_n, staff_n);
        case 'beam' :
          return me.processBeam(element, staff, parent_layer,
              measure_n, staff_n);
        case 'chord' :
          return me.processChord(element, staff, parent_layer,
              measure_n, staff_n);
        case 'anchoredText' :
          return me.processAnchoredText(element, staff, parent_layer,
              measure_n, staff_n);
        default :
          throw new m2v.RUNTIME_ERROR('BadArguments',
              'Rendering of element "' + element_type
                  + '" is not supported.');
      }
    },
    ...
    getClefForStaffNr : function(staff_n) {
      ...
      staff_info = me.currentStaffInfos[staff_n];
      if (!staff_info) {
        throw new m2v.RUNTIME_ERROR('MEI2VF.getClefForStaffNr():E01',
            'No staff definition for staff n=' + staff_n);
      }
      ...
      try {
        ...
        if (note_staff_n === staff_n) {
          ...
          if (otherStaff) {
            ...
            throw new m2v.RUNTIME_ERROR('Error',
                'Note has staff attribute "' + note_staff_n
        ...
        try {
          ...
          throw new m2v.RUNTIME_ERROR('BadArguments',
              'A problem occurred processing the dots of <note>: '
                  + m2v.util.listAttrs(element));
        }

        if (mei_accid) me.processAttrAccid(mei_accid, note, 0);
        if (mei_ho) me.processAttrHo(mei_ho, note);

        $.each($(element).find('artic'), function(i, ar) {
              note.addArticulation(0, new VF.Articulation(m2v.tables.articulations[$(ar)
                  .attr('artic')]).setPosition(m2v.tables.positions[$(ar).attr('place')]));
            });
        ...
        if (!pname) throw new m2v.RUNTIME_ERROR('MEI2VF.RERR.BadArguments',
              'mei:note must have pname attribute');
        if (!oct) throw new m2v.RUNTIME_ERROR('MEI2VF.RERR.BadArguments',
              'mei:note must have oct attribute');

        if (mei_tie) me.processAttrTie(mei_tie, xml_id, pname, oct, me.currentSystem);
        if (mei_slur) me.processAttrSlur(mei_slur, xml_id, pname, oct, me.currentSystem);
        ...
        throw new m2v.RUNTIME_ERROR('BadArguments', 'A problem occurred processing the <note>: '
    ...
    processChord : function(element, staff, parent_layer, measure_n, staff_n) {
      ...
      try {
        ...
        for (i = 0, j = $children.length; i < j; i += 1) {
          ...
          if ($children[i].getAttribute('dots') === '1') hasDots = true;
        }

        // TODO handle dots with ledger lines (has to be fixed in
        // VexFlow)
        if (hasDots) duration += 'd';
        ...
        $children.each(function(i, mei_note) {
              ...
              if (note_mei_tie) me.processAttrTie(note_mei_tie, note_xml_id,
                    note_atts.pname, note_atts.oct, me.currentSystem);
              if (mei_slur) me.processAttrSlur(note_mei_slur, note_xml_id,
                    note_atts.pname, note_atts.oct, me.currentSystem);
              ...
              if (mei_accid) me.processAttrAccid(mei_accid, chord, i);
            });

        if (hasDots) chord.addDotToAll();
        if (mei_ho) me.processAttrHo(mei_ho, chord);
    ...
    processRest : function(element, staff, parent_layer, measure_n, staff_n) {
      ...
      try {
        ...
        if (mei_ho) me.processAttrHo(mei_ho, rest);

        rest.setStave(staff);
        if ($(element).attr('dots') === '1') rest.addDotToAll();
        return rest;
      } catch (e) {
        throw new m2v.RUNTIME_ERROR('BadArguments', 'A problem occurred processing the <rest>: '
                + m2v.util.listAttrs(element)); 
      }
    },
    ...
    processmRest : function(element, staff, parent_layer, measure_n, staff_n) {
      ...
      try {
        ...
        mei_ho = $(element).attr('ho');
        if (mei_ho) me.processAttrHo(mei_ho, mRest);
        mRest.setStave(staff);
        return mRest;
      } catch (x) {
        throw new m2v.RUNTIME_ERROR('BadArguments', 'A problem occurred processing the <mRest>: '
                + m2v.util.listAttrs(element));
      }
    },
    ...
    processSpace : function(element, staff, parent_layer, measure_n, staff_n) {
      var me = this, space;
      try {
        space = new VF.GhostNote({duration : me.processAttsDuration(element, true) + 'r'});
        space.setStave(staff);
        return space;
      } catch (e) {
        throw new m2v.RUNTIME_ERROR('BadArguments',
            'A problem occurred processing the <space>: '
                + m2v.util.listAttrs(element));
      }
    },
    ...
    processAttrAccid : function(mei_accid, vexObject, i) {
      ...
      if (!val) {
        throw new m2v.RUNTIME_ERROR('MEI2VF.RERR.BadAttributeValue',
            'Invalid attribute value: ' + mei_accid);
      }
    ...
    parse_slur_attribute : function(slur_str) {
      ...
      for (i = 0, j = numbered_tokens.length; i < j; i += 1) {
        ...
        if (numbered_token.length === 1) {
          ...
          if (!num) {
            throw new m2v.RUNTIME_ERROR(
                'MEI2VF.RERR.BadArguments:ParseSlur01',
                "badly formed slur attribute");
          }
          result.push({
    ...
    processAttsPitch : function(mei_note) {
      ...
      if (!pname || !oct) {
        throw new m2v.RUNTIME_ERROR('MEI2VF.RERR.MissingAttribute',
            'pname and oct attributes must be specified for <note>');
      }
      return pname + '/' + oct;
    },
    ...
    addDirections : function(note, xml_id) {
      ...
      while (i--) {
        ...
        if (thisDir.startid === xml_id) {
          note.addAnnotation(2, thisDir.place === 'below'
                  ? me.createAnnot(thisDir.text, me.cfg.annotFont).setBottom(true)
                  : me.createAnnot(thisDir.text, me.cfg.annotFont));
        }
      }
    },
    ...
    processSyllables : function(note, element, staff_n) {
      ...
      if (syl) {
        ...
        if (syl.wordpos) me.hyphenation.addSyllable(annot, syl.wordpos, staff_n);
      } else {
    ...
    createAnnot : function(text, annotFont) {
      var me = this;
      return (new VF.Annotation(text)).setFont(annotFont.family,
          annotFont.size, annotFont.weight);
    },
    ...
    getMandatoryAttr : function(element, attribute) {
      ...
      if (!result) {
        throw new m2v.RUNTIME_ERROR('MEI2VF.RERR.MissingAttribute',
            'Attribute ' + attribute + ' is mandatory.');
      }
      return result;
    },
    ...
    translateDuration : function(mei_dur) {
      ...
      throw new m2v.RUNTIME_ERROR('BadArguments', 'The MEI duration "'
              + mei_dur + '" is not supported.');
    },
    ...
    processAttsDuration : function(mei_note, noDots) {
      ...
      if (!noDots && $(mei_note).attr('dots') === '1') dur += 'd';
      return dur;
      ...
      while (i--) {
        if (allVexMeasureStaffs[i]) {
          ...
          while (k--) {
            ...
            if (staff) max_start_x = Math.max(max_start_x, staff.getNoteStartX());
          }
          k = allVexMeasureStaffs[i].length;
          while (k--) {
            staff = allVexMeasureStaffs[i][k];
    ...
    getFirstDefinedStaff : function(staffsInMeasure) {
      ...
      while (i--) {
        if (staffsInMeasure[i]) return i;
      }
      throw new m2v.RUNTIME_ERROR('ERROR',
          'getFirstDefinedStaff(): no staff found in the current measure.');
    },
    ...
    drawVexVoices : function(allStaffVoices, ctx) {
      ...
      for (i = 0, j = allStaffVoices.length; i < j; i += 1) {
        ...
        if (!firstDefined) firstDefined = me.getFirstDefinedStaff(staffsInMeasure);
        allStaffVoices[i].staveVoices
            .format(staffsInMeasure[firstDefined].getNoteEndX()
                - staffsInMeasure[firstDefined].getNoteStartX()
                - me.cfg.measurePaddingRight);
