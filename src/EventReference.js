 ...
 * 04.07.2013
 * 
 * Represents and event with its xmlid, but if the xmlid is not defined, it can
 * also hold the timestamp that can be resolved as soon as the context that
 * holds the event is established. When the tstamp reference is being resolved,
 * the xml:id is calculated using the generic function tstamp2id(), then the
...
var MEI2VF = (function(m2v, VF, $, undefined) {
  ...
  m2v.EventReference.prototype = {
    ...
    tryResolveReference : function(strict) {
      ...
      if (!tstamp) {
        throw new m2v.RUNTIME_ERROR('MEI2VF:RERR:BADARG:EventRef001',
            'EventReference: tstamp must be set in order to resolve reference.');
      }
      if (this.meicontext) {
        // look up event corresponding to the given tstamp (strictly or
        // losely)
        this.xmlid = MeiLib.tstamp2id(this.tstamp,
            this.meicontext.layer, this.meicontext.meter);
      } else {
        this.xmlid = null;
      }
    },
