  ...
  m2v.Texts.prototype = {
    ...
    draw : function() {
      ...
      while (i--) {
        ...
        $.each(me.complexTextModels[i].texts, function(i, lineObj) {
              ...
              maxFontSizeInLine = Math.max(me.drawCenterTexts(
                      centerTexts, coords), me
                      .drawRightAlignedTexts(rightTexts,
                          coords), me
                      .drawLeftAlignedTexts(leftTexts,
                          coords));
    ...
    drawCenterTexts : function(centerTexts, coords) {
      ...
      $.each(centerTexts, function(j, obj) {
            ...
            me.ctx.font = fontstyle + ' ' + fontweight + ' '
                + fontsize + 'px Times';
    ...
    drawRightAlignedTexts : function(rightTexts, coords) {
      ...
      while (i--) {
        ...
        fontweight = obj.opts.fontweight || '';
        fontstyle = obj.opts.fontstyle || '';
        me.ctx.font = fontstyle + ' ' + fontweight + ' ' + fontsize
            + 'px Times';

        me.ctx.textAlign = 'right';
        me.ctx.fillText(obj.text, coords.x + coords.w - offsetX,
            coords.y);
        offsetX += me.ctx.measureText(obj.text).width;
      }
      return maxFontSize;
